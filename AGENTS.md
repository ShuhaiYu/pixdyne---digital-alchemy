...
- [ ] NAP fields (name, address, email, ABN) are not duplicated as inline literals anywhere — they live exclusively in `lib/data/business.ts` (`BUSINESS` and `BUSINESS_FORMATTED`). Every other surface imports from there. A grep for the address / email / ABN string outside the constants file (other than CLAUDE.md and AGENTS.md governance copies) is a truth-auditor block. No phone number is published anywhere (owner decision 2026-07-10) — any phone number appearing on a public surface is a truth-auditor block. See CLAUDE.md §14.1.

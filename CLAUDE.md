...
6. **English only.** Owner decision 2026-08-04: these pages carry no translated body copy. Every extra language is another copy that must stay in lockstep with `PrivacyInfo.xcprivacy` and the store privacy labels (rule 4), and both App Review and AdMob read English. An app's own name in another script (好室友) is a brand string, not translated content — that stays. Re-add a translation only if a specific store review actually demands one.
